...
class SaverTest
{
	...
	void save () throws IOException
	{
		Block[] blocks = {
				...
				new Block(Designer.defaultHeight, ShapeType.PARALLELOGRAM, "int[] arr = { 5, 7, 3, 4, 2};", 0),
				new Block(Designer.defaultHeight, ShapeType.HEXAGON, "int i = 0;", 0),
				new Block(Designer.defaultHeight, ShapeType.FUNCTION, "i < arr.size()", 0),
...
//		block4.next = block5;
		...
		try {
			saver.save(testLayout, "src/test/resources/img.png", custom);
		} catch (IOException e) {
			e.printStackTrace();
		}
		//Saver saver = new Saver();
		//saver.save(layout, "src/test/resources/img.png", custom);
	}
}